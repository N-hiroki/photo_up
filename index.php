...
//FileUpload処理　ファイルが送られてきたか判断
if (!isset($_FILES['upfile']['error']) || !is_int($_FILES['upfile']['error']) || !isset($_POST["file_upload_flg"]) || $_POST["file_upload_flg"]!="1") {
  //echo 'パラメータが不正です';
}else{
 $lat = $_POST["lat"];
 $lon = $_POST["lon"];
$tmp_path  = $_FILES["upfile"]["tmp_name"]; //"/usr/www/tmp/1.jpg"アップロード先のTempフォルダ　tempフォルダアドレス取得
...
//-----------------------------------------------------------------------------------
$uniq_name = fileUniqRename($_FILES["upfile"]["name"]); //※上記関数化！
